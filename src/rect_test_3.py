...
def main(): 
    ...
    t = np.linspace(0, (n_samples - 1) / fs, n_samples)
    # 创建正弦波信号
    signal = np.sin(2 * np.pi * f * t) + np.sin(2 * np.pi * 10 * f * t) + np.sin(2 * np.pi * 100 * f * t)
    # 设置 [4800, 9600) 范围内的采样值为 0
    signal[4800:9600] = 0
    draw("sine_wave_48000fs_48000n_100hz_1000hz_10000hz_[4800-9600]_no_window", t, signal, fs, False)
